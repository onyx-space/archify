...
- This fork authoring default: when the request is language-agnostic (no explicit user language choice), author `meta.locale: "zh-CN"` and write title, subtitle, node labels, relationship labels, boundaries, cards, and guided-view notes in Simplified Chinese. Keep product names, code identifiers, commands, protocols, API paths, and environment names in English (see the invariant below). Prefer `en` only when the user explicitly asks for English or the diagram is for an English-first audience. The renderer-owned Viewer UI and authored copy then match—never leave a `zh-CN` authored diagram with an English Viewer.
